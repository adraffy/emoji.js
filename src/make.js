...
let rgi = [];
let rgi_tags = [];
for (let seqs of Object.values(SPEC.seq)) {	
	for (let seq of seqs) {
...
for (let {cps} of SPEC.zwj.RGI_Emoji_ZWJ_Sequence) {
	rgi.push(cps);
}
...
// encode sequences
let enc_rgi = [...new Set(rgi.map(v => v.length))].filter(x => x > 1).sort((a, b) => a - b).map(w => {
...
// size estimation
console.log(enc_rgi.flat().reduce((a, x) => a + x.length, 0))
